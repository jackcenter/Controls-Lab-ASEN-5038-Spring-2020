...
typedef RT_MODEL_State_Space_Quarc_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * State_Space_Quarc(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(State_Space_Quarc_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: State_Space_Quarc -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
